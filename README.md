...
- **자세히보기 상단 3개 탭 스위처**:
  - `[🚌 버스]` `[🍱 식당]` `[🏠 생활]` 직관적 화면 전환
- **🏠 생활 & SKCT 모의 환경 & 편의 기능**:
  - **📝 SKCT 온라인 모의 환경 (화이트보드 & CBT 계산기)**:
    - 실제 온라인 SKCT CBT 시험 환경을 완벽히 모의하는 독립 전용 윈도우 (1000×700 리사이즈 지원)
    - **화이트보드 메모장**: 마우스 필기 펜(색상/굵기 선택), 지우개, **메모장 전체 지우기(Clear)**, 되돌리기(Undo ⌘Z / Redo ⇧⌘Z), **상단바 분·초 타이머(스톱워치 & 카운트다운)**
    - **우측 상단 CBT 계산기**: 캔버스 우측 상단 플로팅 패널, 사칙연산/소수점/백스페이스, 키보드 입력 및 접기/펼치기 지원
  - **한국 표준시(KST) 실시간 시계**
  - **Tabling Spaces 공간 예약 바로가기**
  - **평일 출퇴근 알림**: 평일 08:50(입실 확인) 및 17:50(퇴실 확인) 로컬 시스템 알림 ON/OFF 스위치 토글
  - **개발자 모드 이스터에그**: 생활 탭 버튼 5회 연속 클릭 시 출퇴근 모바일 웹뷰(사내망 Wi-Fi 감지, 17:50 퇴실 게이팅, 입실/퇴실 윈도우) 활성화
