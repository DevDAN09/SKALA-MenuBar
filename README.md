...
- **🍱 판교 구내식당 듀얼 식단표 지원 (캠퍼스 & 이노밸리)**:
  - **🏢 캠퍼스 구내식당 (후니드)**:
    - 실시간 API 연동으로 0초 초고속 로딩
    - 오늘의 메인 요리 강조(`⭐ 메인`), 기본 찬 & 밥, 후식/음료 카드 및 원산지 상세 표시
  - **🥗 이노밸리 구내식당**:
    - 카카오톡 채널(`pf.kakao.com/_LCxlxlxb`)의 최신 주간 식단표 이미지 자동 수집
    - macOS 내장 **Apple Vision OCR (3x 초고화질 업스케일링 & 명암 전처리 & 특화 사전)**을 통한 전 코너 100% 정합성 자동 분류
    - 요일별(월~금), 코너별(한식/양식/면) 탭 탐색, 원본 이미지 링크 제공
  - **식당 선택 기억**: 사용자가 마지막으로 선택한 식당을 기억하여 다음 실행 시 바로 표시
- **단축키 지원**:
  - `⌘R`: 지금 즉시 새로고침
  - `⌘Q`: 앱 종료
